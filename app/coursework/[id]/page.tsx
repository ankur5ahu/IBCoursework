...
export default function Coursework() {
  ...
  return (
    <div className="p-4 min-h-screen flex flex-col justify-center items-center">
      {file ? (
        <div className="grid lg:grid-cols-2 grid-cols-1 gap-4 sm:ml-16 my-12">
          <PDFViewer fileUrl={pdfUrl} title={file.essayTitle} />
          <div className="flex flex-col gap-4">
            <Score
              score={file.score}
              remark={file.remark}
              date={new Date(file.evaluatedDate).toLocaleDateString()}
            />
        ...
        <div className="text-neutrals-500">Coursework not found</div>
      )}
    </div>
  );
}
